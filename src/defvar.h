 ...
 * @brief Fundamental physical constants used in thermochemistry calculations
 *
 * These constants are used throughout OpenThermo for unit conversions and
 * thermodynamic calculations. All values are in SI units unless otherwise specified.
 * @{
 */
...
struct SystemData {
    ...
    int nelevel = 0;                        // Number of electron levels
    std::vector<double> elevel;             // Electron levels (eV)
    std::vector<int> edegen;                // Degeneracy of electron levels
    ...
    int defmass = 3;                        // Mass assignment mode
    int outotm = 0;                         // Output .otm file flag
    int imode = 0;                          // Mode flag
